/* ATHAR Lesson 1 release hardening: safety-transfer mastery + ARIA state support. */
...
act=function(a){
  if(a==='verify'){
    ...
    if(fact===0&&health===0){mark('verification');feedback(copy('<strong>Correct.</strong> Check important claims with good evidence. For medicine, health or safety, involve a trusted adult and do not rely on AI alone.','<strong>صحيح.</strong> تحقق من الادعاءات المهمة بأدلة جيدة. وفي الدواء أو الصحة أو السلامة، استعن بشخص بالغ موثوق ولا تعتمد على الذكاء الاصطناعي وحده.'));complete(false);return}
    feedback(copy('Important information needs checking. Health and safety decisions also need a trusted adult.','المعلومات المهمة تحتاج إلى التحقق. وقرارات الصحة والسلامة تحتاج أيضاً إلى شخص بالغ موثوق.'),true);return;
  }
  return _atharAct(a);
};
...
render=function(){
  ...
  if(progress){progress.parentElement.setAttribute('role','progressbar');progress.parentElement.setAttribute('aria-valuemin','0');progress.parentElement.setAttribute('aria-valuemax','100');progress.parentElement.setAttribute('aria-valuenow',String(Math.round((state.stage+1)*10)));}
